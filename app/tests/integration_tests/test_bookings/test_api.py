...
@pytest.mark.parametrize(
    ...
    [
        ...
        (4, "2030-05-08", "2030-05-22", 10, 409),
    ],
)
async def test_add_and_get_booking(
    ...
    query = await authenticated_ac.get("/bookings")
    assert len(query.json()) == booked_rooms


async def test_del_booking(authenticated_ac: AsyncClient):
